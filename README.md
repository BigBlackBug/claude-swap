...
- **Scripted wave** (`--seq`): you declare the order, the thresholds still decide when to leave each account.
  ...
  Walks the chain from its head — switching there first even if the account you are on still has room — leaves each account when it reaches its threshold, and **skips** any element already spent when its turn comes, so a wave finishes rather than stalling. The last account is where the wave ends: as soon as it is active the run **exits** (it also exits when every account ahead is already spent). The list is the whole route: repeat an account to visit it twice, e.g. `--seq 2 3 2 3 1`, and name the first one again at the end to come back to it (`--seq 1 2 1`). If something outside the wave moves the login — a Claude Code session that outlived a switch rewriting `~/.claude.json`, a manual `/login` or `cswap switch` — the wave judges its own element, not the account the config now names: moved onto a later element, it takes that as its next step; moved anywhere else, it switches back to its element (or on from it, if that element is itself spent). The position lives in the running process, so `--seq` cannot be combined with `--once`.
...
- A Claude Code session that was already running when you switched keeps the old account's token in memory and refreshes it on its own schedule, writing the result over the login cswap just activated. cswap recognises that credential as another slot's, and when it can show the incoming copy is a newer generation of that slot's own login — same token family, later expiry, refresh token not yet past its own deadline — it files it back into that account's backup — on the next usage check, without waiting for you to switch again. Anthropic retires the previous refresh token on every rotation, so the alternative is an account whose stored login is already dead, quarantined as needing a re-login it does not need. When the evidence is missing or contradictory the bytes go to `cswap unclaimed` untouched instead: overwriting a good backup with a worse one costs exactly the account being rescued
...
A row in `usageStatus: "foreign_credential"` — the live credential provably belongs to some other account, so this slot's quota cannot be read — carries an additive `liveCredentialOwner` with the slot number that credential belongs to, when it is one cswap manages and the identity match is uuid-positive. The same drift is what the human view names in `live credential belongs to Account-2 — a switch repairs it`. The field is absent for an unmanaged or unattributable foreign login. cswap keeps the owning slot's stored backup in sync with such a credential on its own, so that account stays pollable and is not falsely quarantined as `relogin_required`; realigning the config still takes a switch. While the drift lasts, the duplicate-credential check compares that slot's *stored backup* rather than the live bytes it is holding, so a drifted config is never reported as `one slot's backup was overwritten` — a real collision still is.
