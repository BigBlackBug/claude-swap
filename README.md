...
- A Claude Code session that was already running when you switched keeps the old account's token in memory and refreshes it on its own schedule, writing the result over the login cswap just activated. cswap recognises that credential as another slot's, and when it can show the incoming copy is a newer generation of that slot's own login — same token family, later expiry, refresh token not yet past its own deadline — it files it back into that account's backup. Anthropic retires the previous refresh token on every rotation, so the alternative is an account whose stored login is already dead. When the evidence is missing or contradictory the bytes go to `cswap unclaimed` untouched instead: overwriting a good backup with a worse one costs exactly the account being rescued
